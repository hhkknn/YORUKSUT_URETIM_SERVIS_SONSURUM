...
{
    public class AddOrUpdateTamYagliAyranOzet
    {
        public Response addOrUpdateTamYagliAyranOzet(TamYagliAyranGunlukOzet tamYagliAyranGunlukOzet, string dbName, string mKodValue)
        {
            ...
            try
            {
                ...
                oRS.DoQuery("Select * from \"@AIF_AYRNOZTTMYGL\" where \"U_UretimTarihi\" = '" + tamYagliAyranGunlukOzet.UretimTarihi + "'");
                ...
                if (oRS.RecordCount == 0) //Daha önce bu partiye kayıt girilmiş mi?
                {
                    ...
                    oGeneralService = oCompService.GetGeneralService("AIF_AYRNOZTTMYGL");

                    oGeneralData = (SAPbobsCOM.GeneralData)oGeneralService.GetDataInterface(GeneralServiceDataInterfaces.gsGeneralData);

                    DateTime dt = new DateTime(Convert.ToInt32(tamYagliAyranGunlukOzet.UretimTarihi.Substring(0, 4)), Convert.ToInt32(tamYagliAyranGunlukOzet.UretimTarihi.Substring(4, 2)), Convert.ToInt32(tamYagliAyranGunlukOzet.UretimTarihi.Substring(6, 2)));

                    oGeneralData.SetProperty("U_UretimTarihi", dt);

                    oGeneralData.SetProperty("U_UretimVardiyasi", tamYagliAyranGunlukOzet.UretimVardiyasi);

                    oGeneralData.SetProperty("U_Aciklama", tamYagliAyranGunlukOzet.Aciklama);


                    //oGeneralData.SetProperty("U_Tarih", dt);

                    oChildrenPaketlemeBilgieri1 = oGeneralData.Child("AIF_AYRNOZTTMYGL1");

                    foreach (var item in tamYagliAyranGunlukOzet.tamYagliPaketlemeBilgileri1s)
                    {
                    ...
                    oChildrenPaketlemeBilgierli2 = oGeneralData.Child("AIF_AYRNOZTTMYGL2");

                    foreach (var item in tamYagliAyranGunlukOzet.tamYagliPaketlemeBilgileri2s)
                    {
                    ...
                    oChildrenGunSonuOzeti = oGeneralData.Child("AIF_AYRNOZTTMYGL3");

                    foreach (var item in tamYagliAyranGunlukOzet.tamYagliGunlukOzets)
                    {
                    ...
                    oRS.DoQuery("Select ISNULL(MAX(\"DocEntry\"),0) + 1 from \"@AIF_AYRNOZTTMYGL\"");
                    ...
                    if (resp != null)
                    {
                        ...
                        //}
                        LoginCompany.ReleaseConnection(connection.number, connection.dbCode,ID);
                        return new Response { Value = 0, Description = "Tam Yağlı Ayran Günlük Özet Girişi Oluşturuldu..", List = null };
                    }
                    else
                    {
                        LoginCompany.ReleaseConnection(connection.number, connection.dbCode,ID);
                        return new Response { Value = -5200, Description = "Hata Kodu - 5200 Tam Yağlı Ayran Günlük Özet Girişi Oluşturulurken Hata Oluştu. " + oCompany.GetLastErrorDescription(), List = null };
                    }
                }
                else
                {
                    ...
                    try
                    {
                        ...
                        LoginCompany.ReleaseConnection(connection.number, connection.dbCode,ID);
                        return new Response { Value = 0, Description = "Tam Yağlı Ayran Günlük Özet Girişi Güncellendi.", List = null };
                    }
                    catch (Exception)
                    {
                        LoginCompany.ReleaseConnection(connection.number, connection.dbCode,ID);
                        return new Response { Value = -5300, Description = "Hata Kodu - 5300 Tam Yağlı Ayran Günlük Özet Girişi Güncellenirken Hata Oluştu. " + oCompany.GetLastErrorDescription(), List = null };
                    }
